...
前端通过环境变量 `API_BASE_URL` 配置后端地址，默认为 `http://localhost:8000`。
...
# Windows PowerShell
$env:API_BASE_URL="http://your_server_ip:8000"
```

**或修改配置文件：**

编辑 `src/main/config/apiConfig.ts`：
...
将 `http://localhost:8000` 改为实际的后端地址。
